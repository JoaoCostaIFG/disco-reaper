...
# Prefix lines the Stoat writer adds before the actual message body
_STOAT_TS_PREFIX_RE = re.compile(r"^#{6} <t:(\d+):D>$")
_STOAT_FORWARDED_PREFIX_RE = re.compile(r"^#{5} ⮫\*forwarded\*$")
...
# Note appended by the Stoat writer when the server blocks an attachment type
_FILETYPE_NOTE_RE = re.compile(r"^\n?⚠ .*file type not allowed", re.IGNORECASE)
...
def _extract_target_body(content: str) -> tuple[str, int | None]:
    """
    Strips the '######' subtext prefix lines added by the Stoat writer
    (timestamp, forwarded marker).
    Returns (body, timestamp_from_prefix or None if the prefix is missing).
    """
    lines = (content or "").split("\n")
    ts_match = _STOAT_TS_PREFIX_RE.match(lines[0]) if lines else None
    ts_val = int(ts_match.group(1)) if ts_match else None
    i = 0
    while i < len(lines) and (
        _STOAT_TS_PREFIX_RE.match(lines[i])
        or _STOAT_FORWARDED_PREFIX_RE.match(lines[i])
    ):
        i += 1
    return "\n".join(lines[i:]), ts_val
...
async def _fetch_all_target_messages(context: MigrationContext, target_channel_id: str) -> Dict[str, Any]:
    """Pages backwards through the Stoat channel and returns {message_id: Message}."""
    messages: Dict[str, Any] = {}
    cursor = None
    try:
        channel = await context.stoat_writer.client.fetch_channel(str(target_channel_id))
        while True:
            if not context.is_running:
                break
            batch = await channel.history(limit=100, before=cursor)
            if not batch:
                break
            for m in batch:
                messages[str(m.id)] = m
            cursor = str(batch[-1].id)  # history returns newest-first; page backwards from the oldest
    except Exception as e:
...
def _target_author_name(tgt: Any) -> str:
    masq = getattr(tgt, "masquerade", None)
    if masq is not None and getattr(masq, "name", None):
        return str(masq.name)
    author = getattr(tgt, "author", None)
...
async def verify_messages(
    ...
    (main channel and threads) against its mapped Stoat counterpart.
    ...
    async def _verify_message(msg: Any, thread: Any = None):
        ...
        if tgt is None:
            _add_issue("missing_on_target", msg, f"Mapped message {mapping} not found on Stoat (deleted?)")
            return

        tgt_content = tgt.content or ""
        ...
        if body.strip() != expected_body.strip():
            # The Stoat writer appends a note when the server blocks attachment file types;
            # treat that as an attachment issue rather than a content mismatch
            if expected_body and body.startswith(expected_body) and _FILETYPE_NOTE_RE.match(body[len(expected_body):]):
                _add_issue("attachment_mismatch", msg, "Attachments blocked by server (file type not allowed)")
                return
            _add_issue("content_mismatch", msg, f"Content differs: expected {expected_body[:80]!r}, got {body[:80]!r}")
            return

        tgt_attachments = getattr(tgt, "attachments", None) or []
    ...
    for mid, tgt in target_msgs.items():
        ...
        if len(stats["issues"]) < _MAX_ISSUE_DETAILS:
            stats["issues"].append({
                ...
                "source_id": None,
                "author": _target_author_name(tgt),
